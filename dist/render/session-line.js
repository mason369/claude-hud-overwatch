...
export function renderSessionLine(ctx) {
    ...
    if (display?.showSessionTokens && ctx.transcript.sessionTokens) {
        ...
        if (total > 0) {
            parts.push(label(`${t('label.sessionTokenPrefix')}: ${formatTokens(total)} (${t('format.in')}: ${formatTokens(st.inputTokens)}, ${t('format.out')}: ${formatTokens(st.outputTokens)})`, colors));
        }
    }
    // Session duration
