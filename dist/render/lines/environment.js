...
function isRecent(timeStr, now, ttlMs) {
...
function parseResearchBlockLine(line) {
    // 格式: "2026-04-10 15:23:06 [BLOCKED] Edit on /path/file.ts (not read first)"
    const match = line.match(/^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) \[BLOCKED\] (\w+) on (.+?) \((.+)\)$/);
    if (!match)
        return null;
...
const SUBAGENT_EVENT_ZH = {
    SubagentStart: "启动",
    SubagentStop: "停止",
};
/** 防护类 hook 详情的中文描述 */
const GUARD_DETAIL_ZH = {
    "effort-max": "恢复推理深度",
    "stop-phrase-guard": "停止短语拦截",
};
/** 事件类 hook 详情的中文描述 */
const EVENT_DETAIL_ZH = {
    "teammate-idle": "队友",
    "task-completed": "任务",
    "auto-format": "自动格式化",
    "post-compact": "压缩注入",
};
function parseSubagentLine(line) {
...
function isAfterSession(lineTime, sessionStartMs) {
...
function getHookStats(sessionStart) {
    ...
    let latestResearchBlock = null;
    let latestSubagent = null;
    try {
