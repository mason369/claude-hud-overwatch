...
const HOOKS_FIXTURES_DIR = fileURLToPath(
...
function skipIfSpawnUnavailable(result, t) {
  ...
  if (code === "ENOENT") {
    t.skip(
      `required executable is unavailable: ${result.error?.message ?? code}`,
    );
    return true;
  }
...
async function waitForFileContains(filePath, expectedText, timeoutMs = 2000) {
  ...
  throw new Error(
    `Timed out waiting for ${filePath} to contain ${expectedText}`,
  );
}
...
function getCompletionGatePendingEditsFlagPath(
  homeDir,
  sessionId,
  transcriptPath,
) {
...
async function prepareHookHome(homeDir) {
  ...
  for (const entry of await readdir(HOOKS_FIXTURES_DIR, {
    withFileTypes: true,
  })) {
    if (!entry.isFile()) {
      continue;
...
test("runHook executes the copied hook script from the temp HOME", async (t) => {
  ...
  try {
    ...
    await writeExecutable(
      path.join(getHooksDirForHome(homeDir), "task-completed-gate.sh"),
      [
        "#!/usr/bin/env bash",
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || "copied hook should execute successfully",
    );
...
test("task-completed hook writes session metadata into counter and event logs", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || "task-completed hook should succeed",
    );
...
test("completion-gate detects .NET solution roots that only contain a .sln file", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-completion-home-"),
  );
  const projectDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-completion-project-"),
  );
  ...
  try {
    ...
    await mkdir(
      path.dirname(
        getCompletionGatePendingEditsFlagPath(
          homeDir,
          sessionId,
          transcriptPath,
        ),
    ...
    await writeFile(
      getCompletionGatePendingEditsFlagPath(homeDir, sessionId, transcriptPath),
      "dirty",
      "utf8",
    );
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || result.stdout || "completion-gate should succeed",
    );
    assert.match(
      await readFile(markerPath, "utf8"),
      /^test\b/,
      "expected completion-gate to run dotnet test",
    );
    ...
    assert.ok(
      harnessEvents.includes('"category":"tests_passed"'),
      harnessEvents,
    );
...
test("completion-gate detects a working npm test script from a Windows cwd", async (t) => {
  ...
  try {
    ...
    await writeFile(
      path.join(projectDir, "smoke.test.js"),
      [
        "import test from 'node:test';",
        "import assert from 'node:assert/strict';",
        "",
...
test("auto-format uses bunx resolved from PATH instead of a workstation-specific absolute path", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || result.stdout || "auto-format should succeed",
    );
    const bunxArgs = await readFile(markerPath, "utf8");
    assert.match(
      bunxArgs,
      /^prettier --write /,
      `expected PATH bunx shim to be used, got: ${bunxArgs}`,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
    restoreEnvVar("USERPROFILE", originalUserProfile);
...
test("research-first guard keeps read tracking session-scoped", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-research-home-"),
  );
  const workspaceDir = path.join(homeDir, "workspace");
  const originalHome = process.env.HOME;
  const originalUserProfile = process.env.USERPROFILE;
  ...
  try {
    ...
    assert.equal(
      readResult.status,
      0,
      readResult.stderr || "read-tracker should succeed",
    );
    ...
    assert.equal(
      editResult.status,
      2,
      `expected edit from another session to be blocked, got ${editResult.status}\n${editResult.stdout}\n${editResult.stderr}`,
    );
    ...
    assert.ok(
      violations.includes('"transcript_path":"C:/tmp/session-editor.jsonl"'),
      violations,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("research-first guard blocks Write on an existing file without requiring a prior Read", async (t) => {
  ...
  try {
    ...
    assert.equal(
      writeResult.status,
      2,
      `Write on an existing scratch file should require a prior Read\n${writeResult.stdout}\n${writeResult.stderr}`,
    );
    assert.match(writeResult.stderr, /read the file now/i);
  } finally {
    restoreEnvVar("HOME", originalHome);
    restoreEnvVar("USERPROFILE", originalUserProfile);
    await rm(homeDir, { recursive: true, force: true });
...
test("research-first guard allows Write on an existing file after a prior Read in the same session", async (t) => {
  ...
  try {
    ...
    assert.equal(
      writeResult.status,
      0,
      `Write should be allowed after a prior Read in the same session\n${writeResult.stdout}\n${writeResult.stderr}`,
    );
...
test("subagent logger writes session metadata for lifecycle events", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-subagent-home-"),
  );
  const originalHome = process.env.HOME;
  const originalUserProfile = process.env.USERPROFILE;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || "subagent logger should succeed",
    );
    ...
    assert.ok(
      content.includes('"transcript_path":"C:/tmp/subagent-session.jsonl"'),
      content,
    );
...
test("completion-gate runs npm test without injecting extra argv", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `completion-gate should allow npm tests without extra argv\n${result.stdout}\n${result.stderr}`,
    );
    ...
    assert.ok(
      harnessContent.includes('"category":"tests_passed"'),
      harnessContent,
    );
...
test("completion-gate returns a structured Stop block when tests fail", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-completion-fail-home-"),
  );
  const workspaceDir = path.join(homeDir, "workspace");
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `completion-gate should return structured block JSON instead of a hook error\n${result.stdout}\n${result.stderr}`,
    );
    ...
    assert.ok(
      harnessContent.includes('"category":"tests_failed"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("completion-gate only runs after an Edit or Write in the current session and clears the pending marker after running", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-completion-dirty-home-"),
  );
  const workspaceDir = path.join(homeDir, "workspace");
  ...
  try {
    ...
    const stopInput = (activeSessionId, activeTranscriptPath) =>
      JSON.stringify({
    ...
    assert.equal(
      ...
      initialStop.stderr ||
        initialStop.stdout ||
        "completion-gate should skip when no edits happened in this session",
    );
    assert.equal(initialStop.stdout, "");
    assert.equal(initialStop.stderr, "");
    assert.equal(
      existsSync(runMarkerPath),
      false,
      "tests should not run before any edit/write marker exists",
    );
    ...
    assert.equal(
      ...
      markDirty.stderr ||
        markDirty.stdout ||
        "auto-format should succeed while marking the session dirty",
    );
    ...
    assert.equal(
      otherSessionStop.status,
      0,
      otherSessionStop.stderr ||
        otherSessionStop.stdout ||
        "completion-gate should not reuse a different session marker",
    );
    assert.equal(
      existsSync(runMarkerPath),
      false,
      "tests should remain skipped for a different session",
    );

    const sessionFlagPath = getCompletionGatePendingEditsFlagPath(
      homeDir,
      sessionId,
      transcriptPath,
    );
    assert.equal(
      existsSync(sessionFlagPath),
      true,
      "auto-format should mark the current session as having pending edits",
    );
    ...
    assert.equal(
      ...
      runStop.stderr ||
        runStop.stdout ||
        "completion-gate should run tests after an edit/write in the same session",
    );
    ...
    assert.equal(
      existsSync(sessionFlagPath),
      false,
      "completion-gate should clear the pending edit marker after running tests",
    );
    ...
    assert.ok(
      harnessContent.includes('"category":"tests_running"'),
      harnessContent,
    );
    assert.ok(
      harnessContent.includes('"category":"tests_passed"'),
    ...
    assert.equal(
      ...
      secondStop.stderr ||
        secondStop.stdout ||
        "completion-gate should skip after the pending marker has been cleared",
    );
...
test("stop-phrase-guard returns a structured Stop block using last_assistant_message", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-stop-phrase-home-"),
  );
  const originalHome = process.env.HOME;
  const originalUserProfile = process.env.USERPROFILE;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `stop-phrase-guard should return structured block JSON instead of a hook error\n${result.stdout}\n${result.stderr}`,
    );
    ...
    assert.match(
      parsed.reason ?? "",
      /permission|continue working|stop phrase/i,
    );
    ...
    assert.ok(
      harnessContent.includes('"category":"phrase_detected"'),
      harnessContent,
    );
...
test("auto-format reports skipped instead of claiming success when no dotnet workspace is available", async (t) => {
  ...
  try {
    ...
    assert.equal(
      ...
      0,
      result.stderr ||
        "auto-format should not crash when no formatter workspace exists",
    );
    assert.doesNotThrow(
      () => JSON.parse(result.stdout),
      `expected hookSpecificOutput JSON, got: ${result.stdout}`,
    );
    const parsed = JSON.parse(result.stdout);
    ...
    assert.ok(
      harnessContent.includes('"category":"format_skipped"'),
      harnessContent,
    );
    assert.ok(
      !harnessContent.includes('"category":"format_applied"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("safety-gate allows destructive root deletion commands because safety enforcement is disabled", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `safety-gate should no longer block Bash commands\n${result.stdout}\n${result.stderr}`,
    );
    ...
    assert.ok(
      harnessContent.includes('"category":"safe_command"'),
      harnessContent,
    );
    assert.ok(
      !harnessContent.includes('"category":"recursive_delete"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
  ...
  try {
    ...
    const result = runHook("safety-gate.sh", {
      input: JSON.stringify({
        ...
        tool_input: {
          command:
            "python3 -c \"import sqlite3; conn=sqlite3.connect('local.db'); conn.execute('DROP TABLE IF EXISTS nodes_fts')\"",
        },
      }),
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      0,
      `safety-gate should allow local sqlite repair commands\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
    ...
    assert.ok(
      !harnessContent.includes('"category":"sql_destructive"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("linter protection allows edits to lint and formatter config files", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `linter protection should no longer block config edits\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
  } finally {
...
test("agent-opus enforcer allows agents that explicitly request opus", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-agent-opus-home-"),
  );
  const originalHome = process.env.HOME;
  const originalUserProfile = process.env.USERPROFILE;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    const result = runHook("agent-opus-enforcer.sh", {
      input: JSON.stringify({
        tool_name: "Agent",
        model: "opus",
        session_id: "session-agent-opus-pass",
        transcript_path: "C:/tmp/session-agent-opus-pass.jsonl",
      }),
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      0,
      `agent-opus should allow explicit opus model\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
    ...
    assert.ok(
      harnessContent.includes('"category":"model_check"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("agent-opus enforcer accepts the current Agent payload shape with tool_input.model", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-agent-opus-home-"),
  );
  const originalHome = process.env.HOME;
  ...
  try {
    ...
    const result = runHook("agent-opus-enforcer.sh", {
      input: JSON.stringify({
        ...
        session_id: "session-agent-opus-nested",
        transcript_path: "C:/tmp/session-agent-opus-nested.jsonl",
      }),
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      0,
      `agent-opus should accept the current nested Agent payload shape\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
...
test("agent-opus enforcer blocks non-opus model overrides", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-agent-opus-home-"),
  );
  const originalHome = process.env.HOME;
  ...
  try {
    ...
    const result = runHook("agent-opus-enforcer.sh", {
      input: JSON.stringify({
        tool_name: "Agent",
        model: "sonnet",
        session_id: "session-agent-opus-block",
        transcript_path: "C:/tmp/session-agent-opus-block.jsonl",
      }),
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      2,
      `agent-opus should hard-block non-opus overrides\n${result.stdout}\n${result.stderr}`,
    );
    assert.match(result.stdout, /model must be explicitly set to opus/i);
    assert.match(result.stdout, /current:\s*sonnet/i);
    assert.equal(result.stderr, "");
    ...
    assert.ok(
      harnessContent.includes('"category":"model_override"'),
      harnessContent,
    );
    assert.ok(harnessContent.includes('"detail":"sonnet"'), harnessContent);
  } finally {
    restoreEnvVar("HOME", originalHome);
    restoreEnvVar("USERPROFILE", originalUserProfile);
    await rm(homeDir, { recursive: true, force: true });
...
test("agent-opus enforcer blocks agents that omit an explicit model", async (t) => {
  const homeDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-agent-opus-home-"),
  );
  const originalHome = process.env.HOME;
  ...
  try {
    ...
    const result = runHook("agent-opus-enforcer.sh", {
      input: JSON.stringify({
        tool_name: "Agent",
        session_id: "session-agent-opus-missing",
        transcript_path: "C:/tmp/session-agent-opus-missing.jsonl",
      }),
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      2,
      `agent-opus should require an explicit opus model declaration\n${result.stdout}\n${result.stderr}`,
    );
    assert.match(result.stdout, /model must be explicitly set to opus/i);
    assert.match(result.stdout, /missing model/i);
    assert.equal(result.stderr, "");
    ...
    assert.ok(
      harnessContent.includes('"category":"model_missing"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("settings allow unrestricted Bash and no longer deny env or secrets access", async () => {
  ...
  assert.ok(
    !allow.some((entry) => /^Bash\(.+\)$/.test(entry)),
    JSON.stringify(allow),
  );
  assert.ok(
    !deny.some((entry) =>
      /(^|\/)\.env(\.\*|\)|$)|secrets\/\*\*|cat \.env|cat \.\/secrets/i.test(
        entry,
      ),
    ),
    JSON.stringify(deny),
...
test("settings do not route Bash through safety-gate or Edit|Write through linter protection", async () => {
  ...
  const safetyGroups = preToolUse.filter(
    (group) =>
      Array.isArray(group?.hooks) &&
      group.hooks.some((hook) =>
        String(hook?.command ?? "").includes("safety-gate.sh"),
      ),
  );
  const linterGroups = preToolUse.filter(
    (group) =>
      Array.isArray(group?.hooks) &&
      group.hooks.some((hook) =>
        String(hook?.command ?? "").includes("linter-config-protection.sh"),
      ),
  );
...
test("settings do not route search or read tools through the cbm discovery gate", async () => {
  const settingsPath = path.join(REAL_HOME, ".claude", "settings.json");
  const settings = JSON.parse(await readFile(settingsPath, "utf8"));
  const cbmGroups = (settings.hooks?.PreToolUse ?? []).filter(
    (group) =>
      Array.isArray(group?.hooks) &&
      group.hooks.some((hook) =>
        String(hook?.command ?? "").includes("cbm-code-discovery-gate"),
      ),
  );
...
test("cbm gate does not block code search tools before MCP discovery is used", async (t) => {
  ...
  try {
    ...
    const result = runHook("cbm-code-discovery-gate", {
      input: JSON.stringify({
        tool_name: "Grep",
        tool_input: { pattern: "Harness" },
        session_id: "session-cbm-before-mcp",
    ...
    assert.equal(
      result.status,
      0,
      `cbm gate should stay advisory for code search tools\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
    ...
    assert.ok(
      harnessContent.includes('"category":"advisory_only"'),
      harnessContent,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
...
test("cbm session reminder says real-time file reads are the source of truth and CBM is optional", async (t) => {
  const homeDir = await mkdtemp(path.join(tmpdir(), "claude-hud-cbm-home-"));
  const originalHome = process.env.HOME;
  const originalUserProfile = process.env.USERPROFILE;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    const result = runHook("cbm-session-reminder", {
      env: { ...process.env, HOME: homeDir, USERPROFILE: homeDir },
    });
    ...
    assert.equal(
      result.status,
      0,
      result.stderr || result.stdout || "cbm session reminder should succeed",
    );
    assert.match(result.stdout, /real-time/i);
    assert.match(result.stdout, /source of truth/i);
    assert.match(result.stdout, /optional|advisory/i);
    assert.doesNotMatch(
      result.stdout,
      /ALWAYS use codebase-memory-mcp tools FIRST/i,
    );
  } finally {
    restoreEnvVar("HOME", originalHome);
    restoreEnvVar("USERPROFILE", originalUserProfile);
...
test("cbm gate allows reading markdown docs before MCP discovery is used", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `cbm gate should not block markdown reads\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
  } finally {
...
test("cbm gate allows code reads before MCP discovery is used", async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `cbm gate should not block code reads\n${result.stdout}\n${result.stderr}`,
    );
    assert.equal(result.stdout, "");
    assert.equal(result.stderr, "");
  } finally {
