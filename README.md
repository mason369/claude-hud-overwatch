...
- **project.ts** — 时长+速度+费用合并到 project 行，终端宽度不足时由 `wrapLineToWidth` 自适应拆行
- **render/index.ts** — 上述自适应合并的集成逻辑
...
> **重要**：Hook 脚本**不随本插件分发**。overwatch 仅读取 hook 产生的日志文件并可视化展示。你需要根据下表自行编写对应的 shell 脚本并放置在 `~/.claude/hooks/` 目录下。

| 脚本 | Hook 类型 | 作用 | 产出日志 |
|------|----------|------|----------|
| `stop-phrase-guard.sh` | Stop | **核心防护**：拦截 5 类共 53 个停止短语，阻止模型停止并注入纠正消息 | `stop-phrase-violations.log` |
| `research-first-guard.sh` | PreToolUse | **先研究再编辑**：阻止对未 Read 过的文件执行 Edit/Write | `research-first-violations.log` |
| `read-tracker.sh` | PostToolUse | 记录 Read 工具读取过的文件路径，供 research-first 检查 | （内部状态文件） |
| `effort-max-enforcer.sh` | PreToolUse | 阻止任何降低 effortLevel 的操作 | `hook-counters.csv` |
| `hook-counter.sh` | （辅助） | 统计各 hook 触发次数 | `hook-counters.csv` |
| `subagent-logger.sh` | SubagentStart/Stop | 记录子代理启动/停止事件 | `subagent.log` |
| `auto-format.sh` | PostToolUse | 代码格式化自动执行 | `hook-counters.csv` |
| `post-compact-reinject.sh` | PostCompact | 压缩后重新注入关键上下文 | `hook-counters.csv` |
| `teammate-idle-gate.sh` | TeammateIdle | 队友空闲事件计数 | `hook-counters.csv` |
| `task-completed-gate.sh` | TaskCompleted | 任务完成事件计数 | `hook-counters.csv` |

所有日志文件位于 `~/.claude/logs/` 目录下。overwatch 的 `environment.ts` 按日期解析这些日志并渲染到状态栏。
...
│  stop-phrase-guard   │  violations.log     │  environment.ts     │
│  research-first      │  hook-counters.csv  │  解析日志 → 渲染    │
│  subagent-logger     │  subagent.log       │  到状态栏           │
└─────────────────────┘                     └─────────────────────┘
```
